...
void EicDircStandaloneSteppingAction::UserSteppingAction(const G4Step * theStep){
  ...
  //  EicDircStandaloneUserTrackInformation* trackInformation
  //    =(EicDircStandaloneUserTrackInformation*)theTrack->GetUserInformation();
  //  EicDircStandaloneUserEventInformation* eventInformation
  //    =(EicDircStandaloneUserEventInformation*)G4EventManager::GetEventManager()
    ...
    if(thePostPoint->GetStepStatus()==fGeomBoundary){
      if(fExpectedNextStatus==StepTooSmall){
        if(boundaryStatus!=StepTooSmall){
          ...
          ed << "EicDircStandaloneSteppingAction::UserSteppingAction(): "
             << "No reallocation step after reflection!"
             << G4endl;
          G4Exception("EicDircStandaloneSteppingAction::UserSteppingAction()", "EicDircStandaloneExpl01",
                      FatalException,ed,
